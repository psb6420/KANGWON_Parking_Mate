...
> ⚠️ **시연·테스트는 iOS(iPhone) 환경에서 진행해 주세요.** 앱(PWA) 전체 기능 시연은 **iOS에서만 정상 동작**합니다. 라이브 데모 주소를 **iPhone Safari**로 열고 → 공유 메뉴 → **홈 화면에 추가** → 추가된 아이콘으로 실행한 뒤, 길찾기/알림 권한을 허용해 주세요. 안드로이드(및 데스크톱 브라우저)에서는 일부 기능(특히 길찾기 중 푸시 알림·카카오내비 연동)이 기대대로 동작하지 않을 수 있어 시연 환경으로 권장하지 않습니다.
...
- **시연·테스트는 iOS(iPhone)에서 진행을 권장합니다.** iOS 16.4+에서 Safari로 라이브 주소를 열고 홈 화면에 추가한 뒤 웹앱으로 실행해야 합니다. 안드로이드는 백그라운드 전달 신뢰도가 환경에 따라 달라 시연 환경으로 권장하지 않습니다.
