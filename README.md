...
추천 카드의 `길찾기` 버튼은 주차장 이름과 WGS84 경도/위도를 `Kakao.Navi.start()`에 전달합니다. 이 기능은 모바일 기기에서 카카오내비 앱을 실행하며, 앱이 설치돼 있지 않으면 설치 페이지로 이동합니다. 웹앱 내부에서 직접 턴바이턴 길 안내를 제공하는 방식은 아닙니다.
